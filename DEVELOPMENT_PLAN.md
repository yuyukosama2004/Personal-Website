...
- [ ] 桌面端和移动端导航正常。
- [ ] Light / Dark 主题正常。
- [ ] Markdown、代码块、目录和标签正常。
- [ ] RSS、Sitemap、robots.txt 正常。
- [ ] 404 和失效链接检查通过。
- [ ] 键盘导航和自动无障碍检查通过。
- [ ] Lighthouse 与性能预算达标或有记录的例外。
...
- [ ] HTTPS 证书有效且自动续期。
- [ ] HTTP 正确跳转 HTTPS。
- [ ] GitHub Actions CI 和 Deploy 成功。
- [ ] 部署使用非 root 用户。
- [ ] 回滚已实际演练。
- [ ] Nginx 配置测试通过且未影响已有站点。
- [ ] 仓库与构建产物没有 Secret。
- [ ] 合规状态已确认。
...
这些问题不阻塞仓库和框架初始化，但应在对应阶段开始前确认：

1. 网站主要显示真实姓名、网名还是双名称。
2. 首页采用中文、英文还是中英混排。
3. 视觉风格、主色、头像和字体。
4. 对外公开的邮箱或其他联系方式。
5. NovelFlow 等私有项目是否显示源码按钮。
6. 哪些项目能够提供在线演示。
