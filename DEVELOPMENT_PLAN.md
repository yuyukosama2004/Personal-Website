...
- 2026-07-20 可见性复核后，网站展示的 7 个项目中有 6 个提供公开源码；AI RSS Stack 仍作为私有实验记录展示。
...
- 实时抓取 GitHub API 数据。
- 自建评论服务。
- 多语言全文版本（已在 v1.1.0 作为上线后增强完成）。
- 邮件订阅和营销自动化。
- 为追求动效而引入大型前端运行时。

除已根据用户反馈完成的中英文双语站外，其余能力只有在真实访问数据证明有需求时才进入后续阶段。
...
PhoneMall 只有完成脱敏截图、架构图和可验证测试摘要后才升到第一位。源码仓库现已公开，但公开可读不等于已经具备演示素材、开源许可证或生产就绪状态。

### 4.3 项目列表页

项目列表分为两组：

- **Featured Projects**：ecc-init、GroundedSeek、PhoneMall。
- **More Work**：NovelFlow。
- **Labs & Experiments**：Guarded Agent Pipeline、AI RSS Stack。
...
1. 项目名称和一句话价值。
2. 当前状态：源码可见性、成熟度、维护状态、演示状态和证据等级。
3. 截图、GIF 或短视频。
4. 要解决的问题。
5. 核心设计与功能。
6. 技术架构和技术栈。
...
- [x] GitHub Profile README 与仓库 README 反向链接检查。
...
- [x] 成对页面 canonical、`hreflang`、语言切换、双语搜索与双语 RSS。
- [x] 证据分级的作品集与 GitHub Profile 展示改造。
...
### M7：Portfolio & GitHub Redesign

| ID | Issue | 优先级 | 状态 | 验收标准 |
| --- | --- | --- | --- | --- |
| WEB-701 | 实现证据分级的作品集改造 | P0 | 已完成 | 定位、项目层级、canonical 和证据标签上线 |
| WEB-703 | 对齐 GitHub Profile 和仓库元数据 | P1 | 已完成 | Profile、网站和仓库入口一致 |
| WEB-702 | 准备 PhoneMall 真实媒体素材包 | P1 | 转入上线后 Backlog | 仅在真实素材齐备后提升项目展示层级 |

### Post-launch Content — 2026-08

| ID | Issue | 优先级 | 状态 |
| --- | --- | --- | --- |
| CONTENT-101 | 发布 ecc-init 声明式 Pack 与可回滚应用文章 | P1 | Backlog |
| CONTENT-102 | 发布静态站原子部署与真实回滚复盘 | P1 | Backlog |
| CONTENT-103 | 更新 ecc-init 案例的安装证据与架构说明 | P1 | Backlog |
| CONTENT-104 | 复盘首月匿名流量并规划下一轮内容 | P1 | Backlog |
| WEB-702 | 准备 PhoneMall 真实媒体素材包 | P1 | Backlog |
...
| M6 | 持续迭代 | 搜索、统计、Profile 与双语站 |
| M7 | 1–2 天 | 作品集改版、GitHub 对齐与发布收口 |
| 上线后内容 | 持续迭代 | 文章、项目证据和月度流量复盘 |
...
| 风险 | 影响 | 应对 |
| --- | --- | --- |
| 公开仓库缺少许可证或成熟度说明 | 访客把“公开可读”误解为“开源、生产可用” | 分开标记源码可见性、许可证、成熟度、证据与演示状态 |
| PhoneMall 缺少真实截图和演示 | 无法提升为首要精选项目 | 保持当前展示层级，真实素材完成前不作更强宣传 |
...
5. 公开仓库显示可核对的源码入口；私有项目只展示公开案例、系统图和工程取舍，不显示失效或不可访问的源码按钮。
6. 评论系统暂缓；匿名 Nginx 聚合日志先用于验证内容与 GitHub CTA 需求。

评论、双语内容和匿名统计的当前决策记录在
[`docs/PRODUCT_DECISIONS.md`](./docs/PRODUCT_DECISIONS.md)，上线与版本验证证据记录在
[`docs/LAUNCH_AUDIT.md`](./docs/LAUNCH_AUDIT.md)。
...
- [x] 规划基线批准并进入 GitHub 管理。
- [x] 创建 M0–M7 Milestones、标签与 Issues。
- [x] 初始化 Astro、TypeScript、pnpm、CI 与内容模型。
- [x] 完成网站、博客、部署、统计、GitHub Profile 和双语改版。
- [x] 发布 v1.0.0 与 v1.1.0，并通过生产 smoke test。
- [x] 将未完成内容统一转入 `Post-launch Content — 2026-08`。
