<!-- 已于 2026-07-16 按网站 v1.1.0 与线上 GitHub Profile 同步。 -->

<p align="right">
  <a href="https://github.com/yuyukosama2004">English</a>
</p>

# 你好，我是 yuyukosama2004 👋

我主要做全栈 AI 应用和开发者工具，常用 Java、Vue、Python、RAG 与 MCP。

我不只关心模型能不能给出答案，也关心数据从哪里来、结果怎么核对、哪些操作需要人确认，以及项目上线后怎样测试、排查和回滚。

- 🌐 [个人网站](https://www.execute42.top/)
- 🧭 [项目案例](https://www.execute42.top/projects/)
- ✍️ [开发笔记](https://www.execute42.top/blog/)

## 精选项目

### [ecc-init](https://github.com/yuyukosama2004/ecc-init)

给现有项目安全地补上 Claude Code 配置和技能；写入前备份，只合并必要改动，并支持回滚。

`Python` `Claude Code` `CLI` `备份` `回滚`

[源码](https://github.com/yuyukosama2004/ecc-init) · [项目案例](https://www.execute42.top/projects/ecc-init/)

### GroundedSeek

把搜索、浏览、引用和版本记录串成一套可以回头核对的研究流程。

`FastAPI` `MCP` `Playwright` `SQLite` `Web Research`

[项目案例](https://www.execute42.top/projects/grounded-seek/)

### PhoneMall — RAG 智能导购商城

一套基于 Spring Cloud 与 Vue 3 的自托管商城，AI 导购会先检索真实商品数据，再回答用户问题。

`Java` `Spring Cloud` `Vue 3` `RAG` `Docker`

[项目案例](https://www.execute42.top/projects/phonemall/)

## 我在意什么

- 把业务建模、界面、接口、数据和部署连成完整产品。
- 让 RAG 与 MCP 基于真实数据工作，而不是只做聊天演示。
- 用测试、日志、备份和回滚降低改动风险。
- 对模型驱动的重要操作保留清楚的人工确认。

部分项目仍在私有仓库中持续开发；公开案例会明确区分已经完成、可以演示和仍在实验的内容。
